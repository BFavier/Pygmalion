...
class ImageObjectDetector(NeuralNetworkClassifier):
    ...
    def __init__(self, in_channels: int,
        ...
        self.downscaling_factor = tuple((s*mp)**len(features) for s, mp in zip(stride, pooling_size or (1, 1)))
        ...
        self.positions = torch.nn.Conv2d(out_features, 4, (1, 1))
    ...
    def forward(self, X: torch.Tensor):
        ...
        return self.detected(X), self.position(X), self.classes(X)
    ...
    def _y_to_tensor(self, x: np.ndarray, y: Iterable[dict],
        ...
        n, h, w = x.shape[:2]
        h, w = (s//f for s, f in zip((h, w), self.downscaling_factor))
        detect = torch.zeros((n, h, w), dtype=torch.bool)
        positions = torch.zeros((n, h, w), dtype=torch.bool)
        classes = torch.zeros((n, h, w), dtype=torch.long)
        for i, d in y:
            for X, Y, W, H, C in zip(*(d[v] for v in "xywh")):
            detect(i, )
